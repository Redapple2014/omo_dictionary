...
export const MainAppNavigator = createBottomTabNavigator(
  {
    ...
    [NAVIGATION_FLASHCARD_STACK_PATH]: {
      ...
      navigationOptions: ({navigation, route}) => {
        let editMode =
          navigation.state?.routes[0]?.params?.edit ||
          navigation.state?.routes[1]?.params?.edit;
        const tabBarVisible =
          !editMode &&
          (navigation?.state?.routes[1]?.routeName == 'NewCardScreen' ||
          navigation?.state?.routes[2]?.routeName == 'NewCardScreen'
            ? false
            : true);
    ...
    [NAVIGATION_FLASHCARD_LIST_STACK_PATH]: {
      ...
      navigationOptions: ({navigation, route}) => ({
        ...
        tabBarVisible:
          navigation.state.index == 1 || navigation.state.index == 2
            ? false
            : true,
      }),
    },
