...
import { View, Text, StatusBar, Platform, TouchableOpacity, StyleSheet, FlatList, Alert } from 'react-native';
...
const FlashcardScreen = (props) => {
                ...
                <View key={index} style={{
                    ...
                    <View
                        ...
                        <View>
                            ...
                            <Text style={{ color: isActive ? Constants.appColors.WHITE : Constants.appColors.BLACK, paddingLeft: editMode && item?.doc?.name != 'Uncategorized' ? -48 : 8, paddingVertical: 4 }}>{item?.doc?.cards.length} {`${t("CardsText")}`}</Text>
                        </View>
                    </View>
                    {
                        editMode &&
    ...
    const handleDelete = () => {
        if (items.length == 0) {
            ...
            Alert.alert(
                `Alert`,
                `Do you want to delete selected cards`,
                [
                    {
                        text: `${t("CancelText")}`,
