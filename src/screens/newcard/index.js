...
const NewCardScreen = (props) => {
  ...
  const addTextInput = (index) => {
    let textInput = JSON.parse(JSON.stringify(definitionTextInput));
    textInput.push(index);
  ...
  const renderData = () => {
    return definitionTextInput.map((value) => {
      return (
        <TextInput
          style={styles.textInput}
          onChangeText={(text) => addValues(text, value)}
        />
      );
    });
  };
      ...
      <ScrollView style={{flex: 1}}>
        ...
        <View
          ...
          <View style={{position: 'absolute', right: 32}}>
            <TouchableOpacity
              onPress={() => addTextInput(definitionTextInput.length)}>
              <Icon
                name="pluscircle"
                size={20}
